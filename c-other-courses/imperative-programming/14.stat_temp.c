...
void mean_variance(const int v[], int n, double *mean, double *variance) {
	for (int i = 0; i < n; i++)
		*mean += v[i] / (double)n;
		for (int i = 0; i < n; i++)
			*variance += (v[i] - *mean) * (v[i] - *mean) / n; //dzięki temu przekazujemy przez referencję ( a nie kopię)
		//i można modyfikować wartość poza int main()
}
...
void pmf(double v[], int trials) {
	...
	for (int i = 0; i < trials; i++) {
		x = rand() % 6; //indeks '0' oznaczania sumę oczek 2, '1' - 3...
		x += rand() % 6; //od razu wynik z drugiej kostki
		v[x] += 1.0 / trials;
	}
}
...
void print_histogram(const double v[], int n, int x_start, double y_scale, char mark) {
	for (int i = 0; i < n; i++) {
		printf("%2i |", i + x_start);//%2i wyrównuje liczby mniejsze niż 2 spacje do prawej strony
