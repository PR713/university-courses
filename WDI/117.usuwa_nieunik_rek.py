...
def dedupe(head):
	...
	if remove:
		...
		head.next = dedupe(head.next)#Jeśli flaga remove jest ustawiona
    #na False, oznacza to, że nie znaleziono duplikatów na tym etapie.
    #Wtedy rekurencja wywoływana jest dla reszty listy, dając
	#nexta do head taki element który zwróci potem wywołanie
	return head
