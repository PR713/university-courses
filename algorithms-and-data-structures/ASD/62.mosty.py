#DFS szukający mosty (lista sąsiedztwa)
...
def bridges(G):
    def DFSVisit(G,u):
        ...
        nonlocal visited#
        nonlocal parent#
        nonlocal low# tych nie trzeba bo to listy i tak widoczne
        time+=1
        times[u]=time #na wykładzie times to 'd'
        visited[u]=True
        anc=[] #lista przodków (krawędzi wstecznych)
        desc=[] #lista potomków (dzieci)
        for v in G[u]:
            if not visited[v]:
                desc.append(v)
                parent[v]=u
                DFSVisit(G,v)
            elif v!=parent[u]: anc.append(v) #czyli jest krawędź wsteczna
        min_u=len(G)+1
        for i in range(len(anc)):
            if times[anc[i]]<min_u: min_u=times[anc[i]]
            #czyli aktualizujemy jeśli istnieje krawędź wsteczna
            #tablica przodków
        min_w=len(G)+1
        for i in range(len(desc)):
            if low[desc[i]]<min_w: min_w=low[desc[i]]
        #szukamy min low(w) dzieci dlatego jeśli nie odwiedzone
        #to dodawaliśmy do listy desc[] potomków
