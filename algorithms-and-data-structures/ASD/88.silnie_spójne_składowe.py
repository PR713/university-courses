#zrobić silnie spójne składowe samemu
...
def DFS(G, n, visited, postVisit, source):
    ...
    postVisit.append(source) #toposort
...
def ConnectedComponents(G, n):
    ...
    for v in range(len(postVisit) - 1, -1, -1): #od końca po toposort
        if visited[postVisit[v]] == False:
            DfsCheck(newG, n, visited, postVisit[v])
            print()
            connectedComp += 1 #czyli nie da się do niego dojść
            #sam DFS bez toposort
    # end for
    print()
