a = int(input('Podaj jaka ma być suma cyfr liczby: '))
def suma(n):
    ans = 0
    while n:
        ans += n % 10
...
first, s, l = 0, 0, 0
while s < a: # skończy na 101, ze znakiem <= przeszła by pętla i s = 102
    x = min(9, a - s)
    #print(x)
...
while not prime(first):
    add = 9
    while suma(first + add) != a:  # gdy będzie równa a pętla się przerwie
    ...
    # bo szybciej przerwie się druga pętla bo suma będzie a i przesuwa się "8" w liczbie
    #print(first) # wypisze daną liczbę znalezioną o sumie a
print(first)